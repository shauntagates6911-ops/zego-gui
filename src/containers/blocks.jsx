...
class Blocks extends React.Component {
    constructor (props) {
        ...
        bindAll(this, [
            ...
            'handlePromptClose',
            'handleCustomProceduresClose',
            'handleKeyDown', // 💣 NUCLEAR FEATURE: Custom Hotkeys
            'onScriptGlowOn',
            'onScriptGlowOff',
            'onBlockGlowOn',
            'onBlockGlowOff',
    ...
    componentDidMount () {
        ...
        // 💣 NUCLEAR FEATURE: Attach global keydown listeners for editor shortcuts
    ...
    // 💣 NUCLEAR FEATURE: Custom hotkey definitions inside workspace
    handleKeyDown (e) {
        if (!this.props.isVisible) return;
        
        // Ctrl + S or Cmd + S: Download project file immediately
        if ((e.ctrlKey || e.metaKey) && e.key === 's') {
            ...
            if (this.props.vm) {
                this.props.vm.saveProjectSb3().then(content => {
                    ...
                    a.href = url;
                    a.download = 'NuclearMod_Project.sb3';
                    a.click();
                });
            }
        }
        
        // Ctrl + Shift + X: Fast Clear Workspace
        if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'X') {
            e.preventDefault();
            if (this.workspace && window.confirm('☢️ Clear all blocks in current workspace?')) {
...
// 💣 UNLOCKED WORKSPACE OPTIONS: Infinite Zoom Range & Grid
Blocks.defaultOptions = {
    zoom: {
        controls: true,
        wheel: true,
        startScale: BLOCKS_DEFAULT_SCALE,
        maxScale: 3.5, // Expanded max zoom!
        minScale: 0.1, // Expanded min zoom out!
        scaleSpeed: 1.1
    },
    grid: {
        spacing: 40,
