...
class SoundTab extends React.Component {
    constructor (props) {
        ...
        bindAll(this, [
            ...
            'handleDuplicateSound',
            'handleExportSound',
            'handleExportAllSounds', // NUCLEAR FEATURE: Batch Export
            'handleClearAllSounds',  // NUCLEAR FEATURE: Wipe Sounds
            'handleNewSound',
            'handleSurpriseSound',
            'handleFileUploadClick',
            'handleSoundUpload',
    ...
    // 💣 NUCLEAR FEATURE: Export ALL sounds in target as a batch zip/sequence
    handleExportAllSounds () {
        ...
        sounds.forEach((sound, idx) => {
            setTimeout(() => {
                ...
                downloadBlob(`${sound.name}.${sound.asset.dataFormat}`, blob);
            }, idx * 200); // Stagger downloads so browser doesn't block them!
        });
    }

    // 💣 NUCLEAR FEATURE: Clear all audio tracks instantly
    handleClearAllSounds () {
        if (!this.props.vm.editingTarget) return;
        const sounds = this.props.vm.editingTarget.sprite.sounds || [];
        while (sounds.length > 0) {
            this.props.vm.deleteSound(0);
        }
        this.setState({selectedSoundIndex: 0});
    }
    ...
    render () {
        ...
        const messages = defineMessages({
            ...
            surpriseSound: {
                defaultMessage: 'Surprise Sound 🎲',
                description: 'Button to get a random sound in the editor tab',
                id: 'gui.soundTab.surpriseSound'
            },
            recordSound: {
                defaultMessage: 'Record Audio 🎙️',
                description: 'Button to record a sound in the editor tab',
                id: 'gui.soundTab.recordSound'
            },
            addSound: {
        ...
        return (
            <AssetPanel
                buttons={[{
                    ...
                    // 💣 UNLOCKED FORMATS: Added .ogg, .flac, .m4a support!
                    title: intl.formatMessage(messages.fileUploadSound),
                    img: fileUploadIcon,
                    onClick: this.handleFileUploadClick,
                    fileAccept: '.wav, .mp3, .ogg, .flac, .m4a',
                    fileChange: this.handleSoundUpload,
                    fileInput: this.setFileInput,
                    fileMultiple: true
                }, {
